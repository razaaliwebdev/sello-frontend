...
const CustomerReview = () => {
  ...
  useEffect(() => {
    ...
    console.log("CustomerReview - Login state:", {
      user: !!user,
      token: !!token,
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useGetTestimonialsQuery({ isActive: true });
  // Also fetch user's pending reviews (if logged in)
  const { data: userPendingReviews, refetch: refetchPending } =
    useGetTestimonialsQuery(
  ...
  return (
    <section className="px-3 sm:px-4 md:px-6 lg:px-8 py-12 bg-[#F5F5F5]">
      <div className="mx-auto flex items-center gap-10 md:flex-row flex-col">
        <div className="md:w-[70%] w-full">
          <div className="flex flex-col md:flex-row items-start md:items-center my-7 justify-between gap-4">
            ...
            <p className="text-gray-500 text-base md:text-lg">
              Rated {averageRating}/5 based on {totalReviews} reviews showing
              our 4 & 5 stars reviews
            </p>
          </div>
                ...
                {isLoggedIn && pendingReviews.length > 0 && (
                  <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <p className="text-yellow-800 text-sm">
                      ...
                      {pendingReviews.length > 1 ? "s" : ""} waiting for admin
                      approval.
                    </p>
                  </div>
                )}
            ...
            <div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-yellow-800 text-sm flex items-center gap-2">
                ...
                <span>
                  ...
                  {pendingReviews.length > 1 ? "They will" : "It will"} appear
                  here once approved by admin.
                </span>
              </p>
            </div>
          )}
            ...
            {token ? (
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-center justify-center sm:justify-start">
                ...
                <button
                  ...
                  className="bg-primary-500 hover:bg-primary-600 text-white px-6 py-3 rounded-lg font-medium transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105 active:scale-95"
                >
